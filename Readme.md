...
- **Editable Options:**
  ...
  - **`ShowMacroPadGui`**: Set to `true` to display the GUI, `false` to hide it completely
- **How to Edit:**
...
- **Visual Interface**
  ...
  - Smooth fade animations with consistent timing (60fps).
  - Can be completely disabled via `ShowMacroPadGui` setting in `numpad_settings.ini`.
...
- **Automatic Cleanup**
  - The Interception driver is automatically closed when the script exits.
  - No need to manually close processes or worry about leftover drivers.
- **Layer Feedback**
  - Tooltips always show current layer information, even when GUI is disabled.
  - Quick visual confirmation of layer changes without needing the full interface.
...
- Check the `ShowMacroPadGui` setting in `numpad_settings.ini` - it should be set to `true`.
